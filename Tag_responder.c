    ...
    while (1)
    {
        ...
        if (status_reg & DWT_INT_RXFCG_BIT_MASK)
        {
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg1, ALL_MSG_COMMON_LEN) == 0 && Anchor_Seq==0)
                {
                ...
                else if (memcmp(rx_buffer, rx_resp_msg2, ALL_MSG_COMMON_LEN) == 0 && Anchor_Seq==1)
                {
                    uint32_t poll_tx_ts, resp_rx_ts, poll_rx_ts, resp_tx_ts;
                    int32_t rtd_init, rtd_resp;
                    float clockOffsetRatio;
                ...
                else if (memcmp(rx_buffer, rx_resp_msg3, ALL_MSG_COMMON_LEN) == 0 && Anchor_Seq==2)
                {
                    uint32_t poll_tx_ts, resp_rx_ts, poll_rx_ts, resp_tx_ts;
                    int32_t rtd_init, rtd_resp;
                    float clockOffsetRatio;
